...
def fetch_category_map(session):
    print("[businessday] Fetching category mapping...")
    category_map = {}
    page = 1
    while True:
        url = f"https://businessday.ng/wp-json/wp/v2/categories?per_page=100&page={page}"
        try:
...
def run_businessday_deep_scrape(is_manual=False, time_threshold=None):
    ...
    if "businessday" not in global_state:
        global_state["businessday"] = {
            "last_seen_guid": None,
            "date": None,
            "today_count": 0
        }
    
    site_state = global_state["businessday"]
    ...
    print(f"Finished businessday! Scraped {new_rows_count} new articles.")
    return new_rows_count

if __name__ == "__main__":
    print("=== BusinessDay Deep API Scraper ===")
    run_businessday_deep_scrape()

...
def fetch_category_map(session):
    print("[punch] Fetching category mapping...")
    category_map = {}
    page = 1
    while True:
        url = f"https://punchng.com/wp-json/wp/v2/categories?per_page=100&page={page}"
        try:
            ...
            break
    print(f"[punch] Successfully mapped {len(category_map)} categories.")
    return category_map

def run_punch_deep_scrape(is_manual=False, time_threshold=None):
    os.makedirs("data", exist_ok=True)
    global_state = load_state()
    
    if "punch" not in global_state:
        global_state["punch"] = {
            "last_seen_guid": None,
            "date": None,
            "today_count": 0
    ...
    api_url = "https://punchng.com/wp-json/wp/v2/posts"
    ...
    try:
        for page in range(1, MAX_PAGES + 1):
            ...
            print(f"[punch] Fetching API page {page}...")
    ...
    def open_new_file(suffix):
        ...
        filename = os.path.join(BASE_DIR, "data", f"{prefix}punch_api_{execution_time_str}")
        if suffix > 0:
    ...
    print(f"Finished punch! Scraped {new_rows_count} new articles.")
    return new_rows_count

if __name__ == "__main__":
    print("=== Punch Deep API Scraper ===")
    run_punch_deep_scrape()
